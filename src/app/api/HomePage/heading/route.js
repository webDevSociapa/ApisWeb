...
export async function PUT(req) {
    try {
        const body = await req.json();
        const { headingContent, _id } = body;

        if (!_id || !headingContent) {
            return NextResponse.json(
                { message: "Both id and headingContent are required." },
                { status: 400 }
            );
        }
        ...
        const result = await collection.updateOne(
            { _id: new ObjectId(_id) }, // Filter by ID
            { $set: { headingContent } } // Update operation
        );
