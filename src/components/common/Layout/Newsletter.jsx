...
const Newsletter = () => {
  ...
  const saveToDatabase = async (email) => {
    ...
    try {
      ...
      const db = client.db("subscribeEmail");
      const collection = db.collection("subscribeData");

      const existing = await collection.findOne({ email });
      if (existing) return "Already subscribed!";

      await collection.insertOne({ email, createdAt: new Date() });
      return "Thank you! Successfully subscribed to our newsletter.";
    } catch (error) {
        ...
        <div className="relative h-[300px] md:h-[448px] bg-[#9F7B49] px-6 py-4">
          ...
          <div className="flex h-full flex-col items-center justify-center bg-white">
            ...
            <p className="text-center font-jost text-[12px] md:text-[20px] font-medium text-[#666666]">
              ...
              offers, and insider insights delivered
            </p>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="flex items-center pt-4 md:pt-10">
                ...
                <button
                  type="submit"
                  className="bg-[#9F7B49] px-2 py-[7px] md:px-3 md:py-[11px] md:text-xl text-xs font-bold text-white"
                >
                  Subscribe
                </button>
              </div>
